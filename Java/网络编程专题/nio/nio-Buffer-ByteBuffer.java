...
-------------------------------
ByteBuffer-字符编码 Charset		|
-------------------------------
	# Charset 类提供了编码与解码的方法
	
		ByteBuffer encode(String str)
			* 对指定的字符串进行编码,把得到的字节存放在一个bytebuffer中
		
		ByteBuffer decode(CharBuffer cb);
			* 对参数 cb指定的字符串缓冲区中的字符进行编码,把得到的字节序列存入 ByteBuffer
		
		CharBuffer decode(ByteBuffer bf);
			* 把bf中的字节序列进行解码,把得到的字符串,放入一个 CharBuffer 中
		
	# 系统预定了很多 Charset 在 StandardCharsets
	
	# Charset 类还有一个静态方法 public static Charset defaultCharset(),返回当前平台的默认字符编码

	# Charset 还可以根据静态方法创建指定编码类型的 Charset 实例对象		
