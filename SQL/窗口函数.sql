	...
	# 开窗函数
		...
		* 排名窗口函数（用于对数据进行分组排名）
			...
			ROW_NUMBER()
				* 为窗口内的每行分配一个唯一的连续整数。
			RANK()
				* 为窗口内的每行分配一个排名，相同值的行拥有相同的排名，后续的排名会跳跃
			DENSE_RANK()
				* 与RANK()类似，但后续的排名不跳跃
			PERCENT_RANK()
				* 以百分比的形式返回当前行在分区中的名次，如果存在名次相同的数据，后续的排名将会产生跳跃。
			CUME_DIST()
				* 计算当前行在分区内的累积分布。
			NTILE()
				* 将分区内的数据分为N等份，并返回当前行所在的分片位置。
		
		* 取值窗口函数
			LEAD()/LAG()
				* 访问窗口内当前行的前后 N 行的值。
				* 回窗口内第N行数据，LAG函数和LEAD函数不支持动态的窗口大小，它们以整个分区作为分析的窗口。
