...
---------------------------
	...
	# 变量声名
		...
		* 如果访问了声明的变量，但是在编译的时候，不能找到声名变量的定义，则会编译失败
...
---------------------------	
	...
	# 类型别名重复定义的问题
		...
		* C++ 标准规定，如果在同一作用域内重新定义已经存在的类型别名，会导致编译错误，因为它被视作重复定义。
		* 如果重复定义相同的类型别名，按标准来说，这是一个错误。
		* 不过，不同的编译器可能会有不同的行为，有的可能会忽略这个错误并继续编译，有的可能会报错。

		typedef int MyInt;
		typedef int MyInt;  // 重复定义，通常应该报错，某些编译器可能并不会报错，只是忽略了第二次的 typedef。
