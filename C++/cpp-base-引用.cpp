...
---------------------
	...
	# 数组引用
		...
		* 指向指针数组的引用
			...
			// 指针数组
			int *intPtrArr[4] = {&idList[0], &idList[1], &idList[2], &idList[3]};

			// 指向指针数组的引用，指针数组有4个元素
			int *(&intPtrArrRef)[4] = intPtrArr;

			// 迭代引用
			for (auto i : intPtrArrRef){
				*i *= *i;
			}
