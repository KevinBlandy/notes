...
-----------------------
    ...
    # 访问父类函数成员
        ...
        * super 只能访问到父类中 public 或者 internal 修饰的方法。
	...
	# 抽象合约
		...
		* 如果合约里面，有一个以上的未实现的函数或者是未给父合约的构造函数提供参数，则当前合约必须标识为抽象的。
		* 包含了完整实现方法的普通合约也可以标记为抽象。
		* 如果构造函数中包含了 storage 引用参数，也必须要声明为抽象合约。
        * 继承了抽象合约，但是没实现所有抽象方法，当前合约也要定义为抽象合约。
		* 抽象合约不能创建实例，只能是作为父合约，被继承后创建实例。
        * 抽象合约也可以继承自其他合约。
        * 可以有构造函数。
        * 抽象合约不能用未实现的函数覆盖已实现的虚函数。
    
    # 接口
        * 使用 interface 关键字定义合约 
        * 类似于抽象合约，但是限制更多
        * 不能继承于其他合约，但是可以继承自接口，与正常的继承规则相同
        * 接口中只能定义未实现的抽象方法，且必须是 external 的
        * 合约中的方法，默认为 virtual 的，覆写的时候不需要使用 override 关键字（非要加，也中）
            * 覆写了接口的方法后，这个方法不能再次被子类覆写，除非明确添加了 virtual 关键字
            ...
            contract B is A {
                ...
                function foo () external  {}
                // 可以被 B 的子类覆写
                function bar () virtual external {}
            }

        * 不能声明状态变量、构造函数、modifier
